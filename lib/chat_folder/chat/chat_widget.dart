...
class _ChatWidgetState extends State<ChatWidget> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      log('User Id ${currentUserReference?.id}');
      log('Reference Path ${currentUserReference?.path}');
      final userPath = currentUserReference?.path;

      // add / to start of path if it does not exist
      final String? userPathWithSlash;
      if (userPath?.startsWith('/') == true) {
        userPathWithSlash = userPath;
      } else {
        userPathWithSlash = '/$userPath';
      }
      log('Reference Path after Slash $userPathWithSlash');
      ...
      queryMessageRecord(
        ...
        queryBuilder: (messageRecord) {
          return messageRecord
              .where(
                'sender',
                isNotEqualTo: userPathWithSlash,
              )
              .where(
                'read',
                isEqualTo: false,
      ...
      ).listen((messageRecords) {
        for (final messageRecord in messageRecords) {
          if (messageRecord.sender?.path != userPath) {
            log('Message Record Sender ${messageRecord.sender}');
            log('User Path $userPathWithSlash');
            if (!messageRecord.read) {
              messageRecord.reference.update({'read': true});
            }
          }
        }
      });
                                                                ...
                                                                Padding(
                                                                  ...
                                                                  child: Text(
                                                                    valueOrDefault<
                                                                      ...
                                                                      listViewMessageRecord
                                                                              .hasTime()
                                                                          ? Jiffy.parseFromDateTime(listViewMessageRecord.time!)
                                                                              .fromNow()
                                                                          : '',
                                                                      '9:41',
                                                                    ),
                                                                    style: FlutterFlowTheme.of(
                                                                        ...
                                                                        .override(
                                                                          fontFamily:
                                                                              'Noto Sans',
                                                                          color: listViewMessageRecord.sender?.id == currentUserReference?.id
